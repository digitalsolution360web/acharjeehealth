...
function DoctorCard({ doc, onBook }: { doc: typeof doctors[0]; onBook: () => void }) {
  return (
    <div style={{
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-6px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 14px 36px rgba(0,0,0,.13)'; }}
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 12px rgba(0,0,0,.07)'; }}
    >
      {/* Specialty badge */}
      <div style={{
        background: '#0e9ab5',
      ...
      {/* Doctor image */}
      <div style={{ position: 'relative', width: '100%', height: 220, background: '#eef6f9', overflow: 'hidden' }}>
        <Image
          src={doc.image}
          alt={doc.name}
          fill
      ...
      {/* Info */}
      <div style={{ padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: 3, flex: 1, borderTop: '1px solid #f0f4f7' }}>
        <h3 style={{ margin: 0, fontSize: 14, fontWeight: 800, color: '#0b2a3a', fontFamily: "'Poppins', sans-serif", lineHeight: 1.3 }}>{doc.name}</h3>
        <p style={{ margin: 0, fontSize: '11px', color: '#64748b', lineHeight: 1.4 }}>{doc.title}</p>
        <p style={{ margin: 0, fontSize: '11px', color: '#64748b', lineHeight: 1.4, fontStyle: 'italic' }}>{doc.specialty}</p>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 'auto', paddingTop: 10 }}>
          <Link
            ...
            style={{
              ...
              height: 30, borderRadius: 4, border: '1.5px solid #15945D',
              color: '#15945D', background: 'transparent', fontSize: '10.5px',
              fontWeight: 700, textDecoration: 'none', fontFamily: "'Poppins', sans-serif",
              whiteSpace: 'nowrap', transition: 'all .2s',
            }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#15945D'; (e.currentTarget as HTMLElement).style.color = '#fff'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = '#15945D'; }}
          >
            View Profile
          </Link>
          <button
            ...
            style={{
              ...
              background: '#15945D', color: '#fff', fontSize: '10.5px',
  ...
  return (
    <>
      ...
      <section style={{
        ...
        <p style={{ color: 'rgba(255,255,255,0.88)', fontSize: 16, maxWidth: 560, margin: '0 auto 0', lineHeight: 1.7 }}>
          Trusted specialists with decades of experience, dedicated to providing world-class healthcare in Asansol &amp; beyond.
        </p>
      </section>


      {/* ── Doctor Slider Section (Screenshot 1 & 3 moving slider) ── */}
      <DoctorSlider />
